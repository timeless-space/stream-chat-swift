...
extension ChatMessage {
    ...
    public func getNotificationTitle() -> String {
        if isWalletRequestPayCell || isRedPacketCell || isRedPacketAmountCell || isRedPacketReceivedCell {
            return "New message"
        } else {
            return "New message from \(author.name ?? "someone")"
        }
    }

    public func getNotificationMessage() -> String {
        if isWalletRequestPayCell {
            guard let payload = attachments(payloadType: WalletAttachmentPayload.self).first else {
                return text
            }
            let userName = author.name ?? ""
            let amount = payload.extraData?.requestedAmount ?? "0"
            let message = "\(userName) is requesting \(amount) ONE"
            return message
        } else if isRedPacketAmountCell {
            let userName = author.name ?? ""
            let amount = extraData.otherReceivedAmount ?? "0"
            let message = "\(userName) just picked up \(amount) ONE!"
            return message
        } else if isRedPacketReceivedCell {
            let userName = author.name ?? ""
            let amount = extraData.topReceivedAmount ?? "0"
            let message = "\(userName) just picked up \(amount) ONE!"
            return message
            ...
            let userName = author.name ?? ""
            let amount = extraData.sentOneTransferAmount ?? "0"
