    ...
    private func configGiftPacket() {
        ...
        let bigInt = BigUInt(amount) {
            if extraData.tokenAddress != nil {
                ...
                amountLabel.text = "\(NumberUtils.formatCurrency(double)) USDC".replacingOccurrences(of: ".0", with: "")
            } else {
                let double = amountFromWeiUnit(amount: bigInt, weiUnit: Constant.oneWeiUnit)
                amountLabel.text = "\(NumberUtils.formatONE(double)) ONE".replacingOccurrences(of: ".0", with: "")
            }
        }
    }
